...
export function computeScreeningResult(
  ...
  for (const stage of SCREENING_STAGES) {
    ...
    for (const resp of sr) {
      ...
      if (resp.result === "correct") {
        const speedRatio = Math.min(
          1,
          resp.reactionTimeMs / (q.timeLimitSeconds * 1000),
        );
    ...
    const normalized =
      sr.length > 0 ? Math.min(100, (rawScore / stage.maxRawScore) * 100) : 0;
    const normalizedWeight = stage.weight / WEIGHT_SUM;

    stageScores.push({
      stageId: stage.id as ScreeningStageId,
      rawScore,
      maxRawScore: stage.maxRawScore,
  ...
  // RAN signal
  const ranAvgMs =
    stageScores.find((s) => s.stageId === "ran")?.avgReactionMs ?? 0;

  // Vowel delta signal (Arabic-specific)
  const veA = responses.filter(
    (r) => r.questionId.endsWith("a") && r.stageId === "vowel_effect",
  );
  const veB = responses.filter(
    (r) => r.questionId.endsWith("b") && r.stageId === "vowel_effect",
  );
